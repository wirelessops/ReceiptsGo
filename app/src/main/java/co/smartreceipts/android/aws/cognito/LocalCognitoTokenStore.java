...
public class LocalCognitoTokenStore {
    ...
    public Cognito getCognitoToken() {
        final String token = mSharedPreferences.get().getString(KEY_COGNITO_TOKEN, null);
        final String identityId = mSharedPreferences.get().getString(KEY_COGNITO_IDENTITY_ID, null);
        final long expirationTimeStamp = mSharedPreferences.get().getLong(KEY_COGNITO_TOKEN_EXPIRATION, -1);
    ...
    public void persist(@Nullable Cognito cognito) {
        final SharedPreferences.Editor editor = mSharedPreferences.get().edit();
